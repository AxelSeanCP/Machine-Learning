...
function predict(inputText){
    // change input review into token
...
function onClick(){
    ...
    // score prediction with 0 to 1 value
    let score = predict(inputText);

    // result condition based on the score
    if(score > 0.5){
...
async function init(){
    // call model tfjs
    model = await tf.loadLayersModel('http://127.0.0.1:5500/Kelas%20Menengah/pertemuan07/tfjs_model/model.json');
    isModelLoaded = true;

    // call word index
    const word_indexjson = await fetch('http://127.0.0.1:5500/Kelas%20Menengah/pertemuan07/word_index.json')
